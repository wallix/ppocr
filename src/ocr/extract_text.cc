    ...
    void operator()(ImageView const & input, unsigned tid, mln::box2d const & box, unsigned button_col)
    {
        ...
        if (font_id == -1u) {
            ...
            for (std::size_t i = 0; i < this->results.size(); ++i, ++first) {
                ...
                display_result(input, tid, box, button_col, *first);
            }
        }
        else {
            std::cout << "-------- font: "
            ...
            display_result(input, tid, box, button_col, this->classifier);
        }
    }
...
inline void usage(char** argv)
{
    ...
    for (unsigned locale_id = 0; locale_id < ocr::fonts::LocaleId::max; ++locale_id) {
        ...
        for (unsigned i = 0; i < ocr::fonts::nfonts[locale_id]; ++i) {
            std::cerr << "  " << i << " - " << ocr::fonts::fonts[0/*TODO*/][i].name << "\n";
        }
        std::cerr << " -1 - all\n";
    }
...
int main(int argc, char** argv)
{
    ...
    Classification clissification(font_id, display_char);
    extract_titles.extract_titles(ImageView(input), ReferenceClassification(clissification));
